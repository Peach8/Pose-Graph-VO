...
%         T = zeros(3,1);         % initial translation
    ...
    methods
        function obj = rgbd_dvo1(varargin)
...
%                 disp([num2str(norm( obj.J * obj.twist)), 'A*twist']);
%                 disp([num2str(norm( obj.J * obj.twist - obj.Jtr)), 'E']);
                % Stop if the step size is too small
                disp(max(norm(obj.omega),norm(obj.v)));
